...
@customElement('mwc-list-2' as any)
export class List extends BaseElement {
  ...
  protected get listElements(): MWCListItem[] {
    return this.slotEl && findAssignedElements(this.slotEl, 'mwc-list-item-2') as MWCListItem[];
  }

  protected get listDividers(): MWCListDivider[] {
    return this.slotEl && findAssignedElements(this.slotEl, 'mwc-list-divider-2') as MWCListDivider[];
  }
    ...
    return {
      ...
      setCheckedCheckboxOrRadioAtIndex: (index, isChecked) => {
        ...
        if (toggleEl && toggleEl.parentElement === listItem) {
          console.log(listItem, toggleEl, toggleEl!.parentElement);
  ...
  protected _getListItemIndex(e: Event) {
    ...
    const nearestParent = closest(eventTarget, `mwc-list-item-2`);

    // Get the index of the element if it is a list item.
    if (nearestParent && matches(nearestParent, `mwc-list-item-2`)) {
